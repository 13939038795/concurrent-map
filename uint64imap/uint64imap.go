...
func New(shards int) *ConcurrentHashMap {
	...
	for i := 0; i < shards; i++ {
		m.HashMap[i] = &ConcurrentMapShared{items: make(map[uint64]interface{})}
	}
	return m
}

// Returns shard under given key
func (m *ConcurrentHashMap) GetShard(key uint64) *ConcurrentMapShared {
	return m.HashMap[uint32(fnv32(strconv.FormatUint(key, 10)))%uint32(m.Shards)]
}

// Sets the given map
func (m *ConcurrentHashMap) MSet(data map[uint64]interface{}) {
...
func (m *ConcurrentHashMap) Set(key uint64, value interface{}) {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
	shard.items[key] = value
...
func (m *ConcurrentHashMap) Upsert(key uint64, value interface{}, cb UpsertCb) (res interface{}) {
	shard := m.GetShard(key)
	shard.Lock()
...
func (m *ConcurrentHashMap) SetIfAbsent(key uint64, value interface{}) bool {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
...
func (m *ConcurrentHashMap) Get(key uint64) (interface{}, bool) {
	// Get shard
	shard := m.GetShard(key)
	shard.RLock()
	// Get item from shard.
...
func (m *ConcurrentHashMap) Has(key uint64) bool {
	// Get shard
	shard := m.GetShard(key)
	shard.RLock()
...
func (m *ConcurrentHashMap) Remove(key uint64) {
	// Try to get shard.
	shard := m.GetShard(key)
	shard.Lock()
...
func (m *ConcurrentHashMap) Pop(key uint64) (v interface{}, exists bool) {
	// Try to get shard.
	shard := m.GetShard(key)
	shard.Lock()
...
// Returns all items as map[uint64]interface{}
func (m *ConcurrentHashMap) Items() map[uint64]interface{} {
...
type IterCb func(key uint64, v interface{})
...
// Return all keys as []uint64
func (m *ConcurrentHashMap) Keys() []uint64 {
	count := m.Count()
...
func (m *ConcurrentHashMap) SetIfPresent(key uint64, newValue, oldValue interface{}) bool {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
	val, ok := shard.items[key]
...
// Sets the given value under the specified key if oldValue was associated with it.
func (m *ConcurrentHashMap) AddIfPresent(key uint64, value interface{}) bool {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
	val, ok := shard.items[key]
